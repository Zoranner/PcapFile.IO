...
| 0.0.2  | 2024-03-19 | 简化数据格式，移除PacketDataHeader层，允许用户直接存储任意字节数组 |
...
- 提供精确的时间戳和同步机制：支持纳秒级的时间精度，确保多源数据的时序一致性
...
工程文件采用标准PCAP格式，用于管理数据文件的索引和元数据。文件结构如下：
...
| - File Entry Offset (4字节): 24          |
| - Time Index Offset (4字节): 24+3*50     |
| - Reserved (2字节): 0                    |
+------------------------------------------+
| 文件条目表 (3个条目，每个50字节)          |
| 条目1:                                   |
...
| 4      | 2          | Version           | 版本号                     |
| 6      | 2          | File Count        | 数据文件数量               |
| 8      | 4          | Total Index Count | 总索引项数                 |
| 12     | 2          | Index Interval    | 索引间隔(毫秒)             |
| 14     | 4          | File Entry Offset | 文件条目表偏移量           |
| 18     | 4          | Time Index Offset | 时间索引表偏移量           |
| 22     | 2          | Reserved          | 保留字段                   |

**索引间隔说明**：
- 索引间隔参数用于控制索引的采样密度，而非要求数据必须按固定间隔写入
- 系统会在构建索引时，按照指定的时间间隔（如1000毫秒）选取采样点
...
| 6      | N          | Relative Path      | 相对路径（UTF-8）  |
| 6+N    | 8          | Start Timestamp    | 起始时间戳         |
| 14+N   | 8          | End Timestamp      | 结束时间戳         |
| 22+N   | 4          | Index Count        | 文件内索引项数量   |
| 26+N   | 4          | Reserved           | 保留字段           |

**文件条目表说明**：
- 文件条目表采用变长记录格式，以适应不同长度的文件路径
- 相对路径字段使用 UTF-8 编码，支持多语言文件名
- 文件ID用于唯一标识文件，在时间范围索引表中引用
- 路径长度字段用于定位后续字段的偏移量
- 所有时间戳采用 UTC 时间，精确到微秒
- 文件大小可通过文件系统直接获取，无需在索引中存储
- 文件内索引表偏移量可通过前面文件的索引项数量计算得到（每个索引项16字节）
...
- 作用原理：记录采样时间点与包含该时间点数据的文件ID之间的映射关系
- 主要优势：
...
| 偏移量 | 长度(字节) | 名称            | 描述         |
|--------|------------|-----------------|--------------|
| 0      | 4          | Checksum        | CRC32 校验和 |
| 4      | 8          | Total Packets   | 总数据包数   |
| 12     | 8          | First Timestamp | 最早时间戳   |
| 20     | 8          | Last Timestamp  | 最晚时间戳   |
| 28     | 4          | Reserved        | 保留字段     |
...
| 数据包1 (变长)                           |
| - 包头 (16字节)                          |
|   * Timestamp Seconds (4字节):           |
|     2024-03-20 11:00:00                 |
|   * Timestamp Microseconds (4字节):      |
|     000000                              |
...
| 12     | 4          | Timestamp Accuracy | 时间戳精度 (微秒)                                  |
...
| 偏移量 | 长度(字节) | 名称                   | 描述                           |
|--------|------------|------------------------|--------------------------------|
| 0      | 4          | Timestamp Seconds      | 数据包捕获时间戳（秒）         |
| 4      | 4          | Timestamp Microseconds | 数据包捕获时间戳（微秒）       |
